...
Daily aggregator of Tennessee-located taildragger classified listings,
...
repos' approach, with the state filter swapped to "TN".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, TN` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Tennessee"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/tennessee/`
...
```html
<iframe
  src="https://taildraggers.github.io/tennessee/"
  title="Taildragger Ads in Tennessee"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_TENNESSEE_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
