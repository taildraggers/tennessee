"""Aggregates Tennessee-located listings from every taildraggers.com
...
ones whose location ends in ", TN".
...
Pennsylvania/Rhode Island/South Carolina/South Dakota repos' approach, with
the state filter swapped to "TN".
"""
...
PAGE_TITLE = "Taildragger Ads in Tennessee"
STATE_ABBR = "TN"
...
_TENNESSEE_LOCATION_RE = re.compile(r",\s*TN\s*$", re.IGNORECASE)


def _is_tennessee(location: str) -> bool:
    return bool(_TENNESSEE_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        tennessee_rows = [row for row in rows if _is_tennessee(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(tennessee_rows)} in Tennessee")
        all_listings.extend(tennessee_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Tennessee listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Tennessee listings: {len(listings)}")
